...
1. **检测 404 错误**：当上游供应商返回 HTTP 404 状态码时，系统识别为「资源未找到」错误
2. **触发故障切换**：系统将该供应商加入当前请求的失败列表，避免重复选择
3. **不计入熔断器**：404 错误不会增加熔断器的失败计数，不影响供应商的健康状态
...
{% callout type="note" title="与其他 4xx 错误的区别" %}
