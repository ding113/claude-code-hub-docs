...
供应商所属的分组标签，支持多标签格式（逗号分隔）。

**格式示例**：

- 单标签：`cli`
- 多标签：`cli,chat,premium`

**匹配规则**：

与用户或 Key 的 `providerGroup` 进行交集匹配。只要存在一个共同标签，即视为匹配成功。
...
当用户或 Key 配置了 `providerGroup` 后，系统启用严格隔离策略：

- 只有标签匹配的供应商可以被选中
- 未设置 `groupTag` 的供应商会被拒绝

{% callout type="warning" %}
如果系统中存在配置了 `providerGroup` 的用户或 Key，请确保相关供应商都设置了正确的 `groupTag`，否则这些用户将无法使用任何供应商。
{% /callout %}
...
`context1mPreference` 是供应商级别的长上下文偏好字段，用来表达“这个供应商是否允许承接长上下文请求”。它本身不是价格开关，也不再代表本地硬编码加价。
...
| 选项值 | 说明 | 使用场景 |
| --- | --- | --- |
| `inherit` | 跟随请求本身的长上下文特征 | 默认推荐 |
| `force_enable` | 优先把该供应商视为可承接长上下文 | 适用于明确支持 1M 的供应商 |
| `disabled` | 不让该供应商承接长上下文 | 适用于只想保留短上下文的供应商 |

### 支持的模型

当前官方长期支持的 1M Claude 模型以 Anthropic 文档为准，常见包括：

| 模型 / 系列 | 说明 |
| --- | --- |
| `claude-opus-4-7` | 官方 1M GA，标准单价 |
| `claude-opus-4-6` | 官方 1M GA，标准单价 |
| `claude-sonnet-4-6` | 官方 1M GA，标准单价 |
| `claude-sonnet-4-5` / `claude-sonnet-4` | 历史 legacy / beta 规则，是否需要额外 header 与是否存在长上下文溢价取决于当期官方文档与价格表 |
...
系统不会因为 `context1mPreference` 或 `context1mApplied` 自动套用固定倍率。实际价格只来自当前价格表：

- 如果价格表存在 `input_cost_per_token_above_200k_tokens` / `*_above_272k_tokens` 等字段，就按这些显式字段计费。
- 如果价格表没有这些字段，长上下文窗口内继续沿用标准单价。

{% callout type="note" title="成本来源" %}
是否加价、阈值是 200K 还是 272K、以及缓存价格如何变化，都应以云端价格表中的显式字段为准，而不是依赖本地固定倍数。
{% /callout %}

### 工作原理

1. **请求判定**：系统根据请求特征与模型价格表识别是否进入长上下文场景。
2. **供应商偏好**：`context1mPreference` 用于表达该供应商是否应参与长上下文候选集合。
3. **实际计费**：真正的单价仍然取自当前价格表的显式字段，而不是由这个偏好字段直接决定。
