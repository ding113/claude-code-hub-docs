...
| 平均响应时间 | 请求的平均处理时长（毫秒） |
| 平均 TTFB | 平均首字节时间（Time To First Byte），单位毫秒 |
| 平均输出速率 | 平均 Token 输出速率（tok/s） |

{% callout type="note" title="v0.3.37 新增" %}
供应商排行榜新增了两个性能指标列：
- **平均 TTFB**：首字节时间，反映供应商的响应速度
- **平均输出速率**：Token 输出速率，反映流式响应的生成速度
{% /callout %}
...
在供应商排行榜中，可以通过类型筛选器快速过滤特定类型的供应商：

| 供应商类型 | 说明 |
|------------|------|
| 全部 | 显示所有供应商 |
| Claude | Anthropic Claude API 供应商 |
| Codex | OpenAI Codex 供应商 |
| Gemini | Google Gemini 供应商 |
| OpenAI | OpenAI Chat Completions 供应商 |

筛选器位于供应商排行榜表格上方，点击可展开下拉菜单选择要显示的供应商类型。
