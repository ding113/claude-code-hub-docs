...
v0.3.33 版本改进了数据库导出的锁管理机制，确保导出操作的稳定性和资源正确释放。
{% /callout %}

##### MonitoredStream 机制

系统使用 `MonitoredStream` 包装器来监控数据库导出流的生命周期：

**工作原理**：
1. 导出开始时获取备份锁，防止多个导出操作并发执行
2. 使用 MonitoredStream 包装 pg_dump 输出流
3. 流正常结束或发生错误时，自动释放备份锁
4. 客户端中断连接时，也能正确释放锁资源
...
|------|------|
| 无锁 | 可以执行导出/导入操作 |
| 备份锁 | 正在进行数据库导出，其他导出/导入操作需等待 |
| 恢复锁 | 正在进行数据库恢复，所有操作需等待 |

{% callout type="warning" %}
如果导出操作因网络问题中断，系统会自动清理锁资源。无需手动干预，几秒后即可重新执行导出操作。
{% /callout %}
