...
Claude Code Hub 提供完整的 Docker Compose 部署方案，通过单一命令即可启动包含
PostgreSQL 数据库、Redis 缓存和 Next.js 应用服务的完整栈。
...
所有服务使用 `unless-stopped`：容器自动重启，除非你显式停止。
...
{% callout type="note" title="调试说明" %}
PostgreSQL 端口在 `docker-compose.yaml` 中已注释。如需本地调试，可取消注释：
...
PostgreSQL 使用自定义 `PGDATA: /data/pgdata` 避免与卷挂载的权限冲突。
{% /callout %}
...
- `DB_POOL_MAX` / `DB_POOL_IDLE_TIMEOUT` / `DB_POOL_CONNECT_TIMEOUT`:
  连接池配置
...
- `FETCH_CONNECT_TIMEOUT` / `FETCH_HEADERS_TIMEOUT` / `FETCH_BODY_TIMEOUT`:
  请求超时配置
- `MAX_RETRY_ATTEMPTS_DEFAULT`: 单供应商最大尝试次数
- `ENABLE_SMART_PROBING` / `PROBE_INTERVAL_MS` / `PROBE_TIMEOUT_MS`:
  智能探测配置
...
- **主 Compose**: `docker-compose.yaml`
- **开发 Compose**: `dev/docker-compose.yaml`
- **Dockerfile**: `deploy/Dockerfile`
- **环境模板**: `.env.example`
- **部署脚本 (Linux/macOS)**: `scripts/deploy.sh`
- **部署脚本 (Windows)**: `scripts/deploy.ps1`
