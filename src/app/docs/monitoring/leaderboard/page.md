...
**用户洞察**：管理员可以点击用户名进入 [用户洞察](/docs/monitoring/user-insights) 页面，查看该用户按模型和供应商拆分的详细统计数据。
...
供应商排行支持按模型拆分的展开行，点击供应商行即可展开查看该供应商下各模型的详细统计。

**展开后显示的模型级指标**：

| 指标 | 说明 |
|------|------|
| 模型名称 | 按 `billingModelSource` 配置解析的模型标识 |
| 请求数 | 该供应商上该模型的请求次数 |
| 成本 | 该模型在该供应商上的消费金额 |
| Token 数 | 输入 + 输出 + 缓存 Token 总和 |
| 成功率 | 该模型在该供应商上的成功率 |
| 平均 TTFB | 该模型的平均首字节时间（毫秒）|
| 平均生成速度 | 该模型的平均每秒 Token 数 |
| 单请求均价 | 总成本 / 总请求数 |
| 百万 Token 均价 | 总成本 * 1,000,000 / 总 Token 数 |

**API 调用方式**：

在排行榜 API 请求中添加 `includeModelStats` 参数：

```bash
GET /api/leaderboard?period=daily&scope=provider&includeModelStats=1
```

{% callout type="note" title="缓存键区分" %}
启用模型统计时，缓存键会追加 `:includeModelStats` 后缀，确保带模型拆分和不带模型拆分的数据分别缓存，互不干扰。
{% /callout %}
