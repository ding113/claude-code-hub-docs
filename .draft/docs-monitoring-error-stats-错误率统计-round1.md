# 错误率统计 (Error Rate Statistics) - Round 1 Exploration Draft
...
export enum ErrorCategory {
  PROVIDER_ERROR,              // 供应商问题（4xx/5xx HTTP 错误）→ 计入熔断器 + 直接切换
...
**检测顺序**（性能优先）：
1. 包含匹配（`String.prototype.includes`）
2. 精确匹配（`Map.get`）
3. 正则匹配（`RegExp.prototype.test`）
...
按供应商聚合错误统计，用于供应商健康度评估：

```typescript
// 从 usage-logs.ts 中的查询逻辑
export async function findUsageLogsStats(filters) {
...
CREATE TABLE message_request (
  ...
  key TEXT NOT NULL,
  model TEXT,
  status_code INTEGER,              -- HTTP 状态码（错误检测依据）
  error_message TEXT,               -- 错误消息（包含限流元数据）
  provider_chain JSONB,             -- 供应商决策链
  duration_ms INTEGER,              -- 请求耗时
  cost_usd DECIMAL(20, 10),         -- 消费金额
  blocked_by TEXT,                  -- 拦截类型（warmup 等）
...
CREATE TABLE error_rules (
  ...
  match_type TEXT NOT NULL,         -- 匹配类型：regex/contains/exact
  category TEXT NOT NULL,           -- 错误类别
  description TEXT,
  override_response JSONB,          -- 覆写响应体
...
| 变量名 | 描述 | 默认值 |
|--------|------|--------|
| `STORE_SESSION_MESSAGES` | 是否存储会话消息（影响错误详情） | `false` |
...
系统预置 30+ 条默认错误规则，位于 `/Users/ding/Github/claude-code-hub/src/repository/error-rules.ts`：
...
| `/Users/ding/Github/claude-code-hub/src/repository/error-rules.ts` | 错误规则数据模型和默认规则 |
...
**Rate Limit Event Query** (`/Users/ding/Github/claude-code-hub/src/repository/statistics.ts:1043-1117`):
...
**RateLimitEventStats Interface** (`/Users/ding/Github/claude-code-hub/src/types/statistics.ts`):
...
2. **规则引擎驱动**：支持 30+ 种预置错误规则，可按正则/包含/精确三种方式匹配
