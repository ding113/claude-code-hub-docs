# Model Redirect (模型重定向) - Round 1 Exploration Draft
...
When integrating with third-party AI providers, especially those in different regions or with different API conventions, model redirects become essential. For example, a Chinese AI provider might use model names like `glm-4.6` or `qwen-max`, while your internal applications expect standard Claude model names.
...
**Original Model Preservation**: The system always preserves the original model name requested by the client. This is stored in the session via `session.setOriginalModel()` before any modification occurs. This preserved name is used for:
- Billing calculations (ensuring users are charged based on what they requested)
- Audit logging and request tracking
- Provider chain decision recording

**Provider-Specific Redirects**: Redirects are configured per-provider, not globally. This allows different providers to handle the same model name differently. For example:
- Provider A might redirect `claude-3-opus` to `gpt-4`
- Provider B might redirect `claude-3-opus` to `glm-4`

**Reset on Provider Switch**: When a request fails and needs to be retried with a different provider, the model is reset to its original value. This ensures that each provider's redirect rules are applied to the original model name, not a previously redirected one.
...
// Regenerate the request buffer with the new model
const updatedBody = JSON.stringify(session.request.message);
const encoder = new TextEncoder();
session.request.buffer = encoder.encode(updatedBody).buffer;
```
...
#### Why Different Handling is Necessary

The different handling exists because:

1. **API Design Differences**: Anthropic and OpenAI use request body parameters, while Google uses URL paths
2. **Request Buffer Integrity**: For body-based providers, we must regenerate the buffer after modification
3. **URL Construction**: For Gemini, we must preserve query parameters and other URL components while only modifying the path
...
In the Drizzle ORM schema, `modelRedirects` is stored as a JSON column:

```typescript
// From drizzle/schema.ts (inferred from repository usage)
...
**Code Reference** (in `resetToOriginal`):
```typescript
private static resetToOriginal(
  session: ProxySession,
  originalModel: string,
...
- `"original"`: Bill using the original model's pricing
- `"redirected"`: Bill using the redirected model's pricing (default)

**Code Reference** (from `/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/response-handler.ts`):
```typescript
...
if (providerChain.length > 0) {
  ...
  lastDecision.modelRedirect = {
    originalModel: originalModel,
    redirectedModel: redirectedModel,
    billingModel: originalModel, // Always use original for billing
  };
}
```
...
**Behavior**: The UI validation prevents this during configuration. However, at runtime:
- Empty source models are treated as "no redirect" (falsy check)
- Empty target models would fail the redirect lookup
...
if (explicitlyDeclared) {
  return true; // Explicit declaration has highest priority
}
```
...
1. **`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/model-redirector.ts`**
   - The main `ModelRedirector` class
   - Contains `apply()`, `getRedirectedModel()`, `hasRedirect()`, and `resetToOriginal()` methods
...
2. **`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/session.ts`**
   ...
   - `setOriginalModel()`, `getOriginalModel()`, `isModelRedirected()` methods
   - Stores `originalModelName` and `originalUrlPathname` for reset functionality

3. **`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/provider-selector.ts`**
   - `providerSupportsModel()` function evaluates if a provider can handle a model
   - Considers both `allowedModels` and `modelRedirects` for support determination
   - Handles `joinClaudePool` logic with redirect validation
...
4. **`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/forwarder.ts`**
   ...
   - Handles provider-specific request preparation after redirect

5. **`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/message-service.ts`**
   - `ProxyMessageService.ensureContext()` saves original model before redirect
...
6. **`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/response-handler.ts`**
   ...
   - Lines 1726-1805 handle billing model selection

### Type Definitions

7. **`/Users/ding/Github/claude-code-hub/src/types/provider.ts`**
   - `Provider` interface defines `modelRedirects: Record<string, string> | null`
   - `CreateProviderData` and `UpdateProviderData` include `model_redirects` field
   - Lines 60, 164, 252, 322

### Validation and Repository

8. **`/Users/ding/Github/claude-code-hub/src/lib/validation/schemas.ts`**
   - Zod schema for `model_redirects` validation
   - Line 383: `model_redirects: z.record(z.string(), z.string()).nullable().optional()`

9. **`/Users/ding/Github/claude-code-hub/src/repository/provider.ts`**
...
10. **`/Users/ding/Github/claude-code-hub/src/app/[locale]/settings/providers/_components/model-redirect-editor.tsx`**
...
```typescript
// From /Users/ding/Github/claude-code-hub/src/types/message.ts (inferred)
...
The billing system uses the original model name by default, but can be configured to use the redirected model name. This is controlled by the `billingModelSource` system setting. The `ProxySession.getCachedPriceDataByBillingSource()` method handles the logic for selecting the appropriate pricing model.
...
```typescript
const targetModel = ModelRedirector.getRedirectedModel(
  "claude-3-opus-20240229",
  provider
);
// Returns "claude-3-sonnet-20240229" if configured, or original if not
```
...
// Returns true if redirect exists, false otherwise
```
...
3. **Billing Integrity**: Original model names are preserved throughout the request lifecycle, ensuring accurate billing and usage tracking based on what the client requested.

4. **Failover Safety**: The system intelligently resets models to their original values during provider failover, ensuring that each provider's redirect rules are evaluated against the original request.
