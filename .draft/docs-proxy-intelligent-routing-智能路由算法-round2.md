...
Step 1: Group Pre-filtering → Step 2: Format/Model Matching → Step 2.5: 1M Context Filter
  ...
  → Step 5: Priority Stratification → Step 6: Cost Sorting → Step 7: Weighted Random Selection
```
...
**Note**: Concurrent session limit checks are performed atomically in the `ensure()` method, not during health filtering, to avoid race conditions.
...
#### Step 6: Cost Sorting
Within the same priority tier, providers are sorted by `costMultiplier` in ascending order (lower cost first).

#### Step 7: Weighted Random Selection
After cost sorting, weighted random selection is applied using `weightedRandom()`:
- Selection probability = provider.weight / totalWeight
- If total weight is 0, falls back to uniform random selection

### 2.2 Session Reuse (Sticky Sessions)

The `findReusable()` method implements session affinity:
- For multi-turn conversations (messages length > 1), the system attempts to reuse the previously bound provider
- Binding is stored in Redis as `session:{sessionId}:provider` with TTL (default from `SESSION_TTL` environment variable, typically 300 seconds)
- Reuse validation includes: provider existence, enabled status, group permission, cost limits, and vendor-type circuit breaker state
...
Session binding TTL: Defined in `SessionManager.SESSION_TTL` (configurable via `SESSION_TTL` environment variable, defaults to 300 seconds / 5 minutes if not set)
Redis keys:
...
This method skips format/model matching and directly filters by `providerType`, making it suitable for model listing endpoints.
