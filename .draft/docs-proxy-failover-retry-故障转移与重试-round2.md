...
```
Outer Loop: Provider Switching (MAX_PROVIDER_SWITCHES attempts, max 20)
  └── Inner Loop: Retry Current Provider (maxAttemptsPerProvider attempts, default 2, max 10)
```
...
- Has a safety limit (`MAX_PROVIDER_SWITCHES = 20`) to prevent infinite loops
...
MAX_RETRY_ATTEMPTS_DEFAULT: z.coerce.number()
  .min(1)
  .max(10)
  .default(2)  // Default: 2 attempts per provider (NOT 3)
```
...
**Default Constant** (`/Users/ding/Github/claude-code-hub/src/lib/constants/provider.constants.ts:22`):
```typescript
...
2. Environment `MAX_RETRY_ATTEMPTS_DEFAULT` (fallback, default 2)
...
export enum ErrorCategory {
  ...
  SYSTEM_ERROR,                // Network errors → Retry with next endpoint, then failover
...
| SYSTEM_ERROR | Configurable | `ENABLE_CIRCUIT_BREAKER_ON_NETWORK_ERRORS` (default: false) |
...
Returns endpoints filtered and sorted by:
1. **Circuit state filter** - Open circuits excluded first (line 44-51)
2. **Probe status** - `lastProbeOk === true` prioritized (rank 0)
3. **Sort order** - Configured `sortOrder` field
4. **Latency** - `lastProbeLatencyMs` ascending

**Note**: Circuit state is a filter (excludes open circuits), not a ranking criterion.
...
| `/Users/ding/Github/lib/webhook/utils/retry.ts` | Generic retry utility |

---

## 11. Key Insights (关键洞察)

1. **Retry is per-provider, not per-request**: The system retries within a provider before considering failover (default: 2 attempts)
...
5. **Network errors are configurable**: Can be excluded from circuit breaker by default (`ENABLE_CIRCUIT_BREAKER_ON_NETWORK_ERRORS` defaults to false)
...
## 12. Corrections from Round 1 (第一轮修正)

### 12.1 Retry Configuration Correction

**Round 1 Error**: Document mentioned "up to 3 retries" as default.

**Correction**: The default is **2** retries per provider (not 3), with a valid range of 1-10. This is defined in:
- `env.schema.ts:113` - `MAX_RETRY_ATTEMPTS_DEFAULT` defaults to 2
- `provider.constants.ts:22` - `MAX_RETRY_ATTEMPTS: 2`

### 12.2 Endpoint Selection Ranking Correction

**Round 1 Error**: Document listed "Circuit state - Closed circuits first" as the first ranking criterion.

**Correction**: Circuit state is actually a **filter** (not a ranking criterion). Open circuits are filtered out before ranking. The actual ranking criteria are:
1. Probe status (`lastProbeOk`) - prioritized first
2. Sort order (`sortOrder`)
3. Latency (`lastProbeLatencyMs`)

See `endpoint-selector.ts:7-28` for the ranking implementation.

---

*Report generated for documentation page: `/docs/proxy/failover-retry`*
*Reviewed and corrected in Round 2*
