# Header Modification (Header 修改) - Round 2 Review Draft

## Review Summary

**Reviewer Notes:**
- Verified all code snippets against actual implementation
- Fixed line number references
- Removed duplicate Section 6 (Testing)
- Corrected test descriptions to match actual Chinese test names
- Verified all file paths exist
- Fixed minor code snippet discrepancies
...
**File**: `src/drizzle/schema.ts` (Lines 508-536)
...
**File**: `src/repository/request-filters.ts` (Lines 8-29)
...
**File**: `src/actions/request-filters.ts`
...
1. Global filters apply first (sorted by priority, then by ID)
2. Provider-specific filters apply second (sorted by priority, then by ID)
...
async applyForProvider(session: ProxySession): Promise<void> {
  ...
  for (const filter of this.providerFilters) {
    ...
    if (filter.bindingType === "providers") {
      ...
      // Optimization #3: O(m) instead of O(m*n), iterate smaller set
...
**File**: `src/app/v1/_lib/proxy/session.ts`
...
**File**: `src/app/v1/_lib/proxy/request-filter.ts`
...
**File**: `src/app/v1/_lib/proxy/provider-request-filter.ts`
...
**File**: `src/app/v1/_lib/headers.ts`
...
**File**: `src/app/v1/_lib/proxy/forwarder.ts` (Lines 2097-2198)
...
The system enforces constraints on binding types (from `src/actions/request-filters.ts`):
...
**File**: `src/types/special-settings.ts` (Lines 73-89)
...
**File**: `src/lib/utils/special-settings.ts` (Lines 113-130)
...
**File**: `tests/unit/proxy/session.test.ts` (Lines 387-438)
...
**File**: `tests/unit/request-filter-engine.test.ts`
...
**File**: `tests/unit/request-filter-binding.test.ts`

Comprehensive test suite covering:

| Category | Description |
|----------|-------------|
| Global Filters | Tests for filters applied to all requests |
| Provider-Specific Filters | Tests for filters bound to specific providers |
| Group-Specific Filters | Tests for filters bound to provider groups |
| Combined Filters | Tests for global + provider + group interaction |
...
**File**: `src/app/[locale]/settings/request-filters/page.tsx`
...
**ProviderMultiSelect** (`src/app/[locale]/settings/request-filters/_components/provider-multi-select.tsx`):
- Loads providers via `listProvidersForFilterAction`
- Checkbox-based selection with select all/clear all

**GroupMultiSelect** (`src/app/[locale]/settings/request-filters/_components/group-multi-select.tsx`):
...
**File**: `drizzle/0024_request-filters.sql`
...
CREATE INDEX IF NOT EXISTS "idx_request_filters_enabled" ON "request_filters" USING btree ("is_enabled","priority");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "idx_request_filters_scope" ON "request_filters" USING btree ("scope");
--> statement-breakpoint
...
**File**: `drizzle/0041_sticky_jackal.sql`
...
| File | Purpose |
|------|---------|
| `src/lib/request-filter-engine.ts` | Core filter engine implementation |
| `src/repository/request-filters.ts` | Type definitions and CRUD operations |
| `src/drizzle/schema.ts` (Lines 508-536) | Database schema |
| `src/app/v1/_lib/proxy/session.ts` | ProxySession with header tracking |
| `src/app/v1/_lib/proxy/request-filter.ts` | Global filter middleware |
| `src/app/v1/_lib/proxy/provider-request-filter.ts` | Provider filter middleware |
| `src/app/v1/_lib/headers.ts` | HeaderProcessor for sanitization |
| `src/app/v1/_lib/proxy/forwarder.ts` | Forwarder with header building |
| `src/actions/request-filters.ts` | Server actions for filter management |
