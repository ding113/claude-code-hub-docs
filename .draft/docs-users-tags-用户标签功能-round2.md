...
**排行榜页面标签筛选**:
- 排行榜使用不同的筛选方式，使用 `?` 操作符检查 JSONB 数组中是否包含指定标签
- 支持标签筛选和用户分组筛选的组合（OR 关系）
...
**数据库迁移** (来自 `drizzle/0023_even_mentor.sql`):
```sql
...
- 默认值为空数组 `[]`，避免 null 处理
- 没有单独的 tags 表，简化数据模型
- 标签筛选使用 PostgreSQL JSONB 操作符 `@>`（包含）和 `?`（存在）
...
**排行榜中的标签筛选** (来自 `/Users/ding/Github/claude-code-hub/src/repository/leaderboard.ts`):
```typescript
...
- 标签的添加、删除
- 自动完成建议（支持字符串或 `{value, label, keywords}` 对象）
- 键盘导航（上下箭头、回车、退格、Escape）
- 粘贴批量添加（支持逗号、中文逗号、换行符分隔）
- 验证回调（`validateTag`、`onInvalidTag`）
- 最大标签数限制（`maxTags`）
- 最大标签长度限制（`maxTagLength`，默认 50）
- 最大可见标签数（`maxVisibleTags`，超出显示 +N 徽章）
- 可清除（`clearable`）
- 禁用状态（`disabled`）
...
- `duplicate`: 重复标签
- `max_tags`: 超出最大标签数
- `invalid_format`: 格式不匹配（默认正则 `/^[a-zA-Z0-9_-]+$/`）

#### 3.3.2 表单字段封装

**文件**: `/Users/ding/Github/claude-code-hub/src/components/form/form-field.tsx`
...
**文件**: `/Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/leaderboard/_components/leaderboard-view.tsx`
...
- 筛选下拉框只显示已存在的标签
- 排行榜中允许输入任意标签（`tagSuggestions.length === 0 || tagSuggestions.includes(tag)`）
...
| `/Users/ding/Github/claude-code-hub/drizzle/0023_even_mentor.sql` | 标签字段的数据库迁移 |
...
| `/Users/ding/Github/claude-code-hub/src/repository/leaderboard.ts` | 排行榜标签筛选实现 |
...
| `/Users/ding/Github/claude-code-hub/src/components/form/form-field.tsx` | 表单字段封装（ArrayTagInputField） |
| `/Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/_components/user/forms/user-form.tsx` | 用户表单 |
| `/Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/_components/user/forms/user-edit-section.tsx` | 用户编辑区块 |
...
| `/Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/leaderboard/_components/leaderboard-view.tsx` | 排行榜页面，标签筛选 |
| `/Users/ding/Github/claude-code-hub/src/app/api/leaderboard/route.ts` | 排行榜 API 路由 |
...
**排行榜标签筛选** (来自 `/Users/ding/Github/claude-code-hub/src/repository/leaderboard.ts`):
```typescript
const normalizedTags = (userFilters?.userTags ?? []).map((t) => t.trim()).filter(Boolean);
let tagFilterCondition: ReturnType<typeof sql> | undefined;
if (normalizedTags.length > 0) {
...
**API Routes**:
- `GET /api/leaderboard?userTags=tag1,tag2`: 获取排行榜，支持用户标签筛选
...
5. **权限控制**: 仅管理员可获取所有标签列表

### 6.3 限制与注意事项

1. **无标签层级结构**: 不支持父子标签关系
2. **无标签权限控制**: 标签本身不控制访问权限
3. **筛选逻辑差异**: 
   - 用户列表页面：标签与分组是 AND 关系
   - 排行榜页面：标签与分组是 OR 关系
4. **标签无独立生命周期管理**: 随用户创建/删除
5. **大量标签可能影响性能**: 建议监控标签数量

### 6.4 Round 1 修正记录

| 项目 | Round 1 | Round 2 修正 |
|------|---------|-------------|
| 排行榜标签筛选 | 未详细说明 | 补充完整实现，包括 `?` 操作符的使用 |
| 标签与分组关系 | 统一为 AND | 区分用户列表（AND）和排行榜（OR） |
| API 端点 | 仅列出 Actions | 补充 `/api/leaderboard` 路由 |
| 数据库迁移 | 未提及 | 添加迁移文件引用 |
| 权限说明 | 较简略 | 明确仅管理员可获取所有标签 |
