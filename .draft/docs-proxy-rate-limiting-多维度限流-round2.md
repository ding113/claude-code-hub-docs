# Rate Limiting Implementation Analysis - Draft Report (Round 2)
...
The system checks rate limits in a carefully designed sequence (`/Users/ding/Github/claude-code-hub/src/app/v1/_lib/proxy/rate-limit-guard.ts`, lines 31-46 and implementation at lines 47-588):
...
**Note**: Provider concurrent session limits are checked separately via `checkAndTrackProviderSession` during provider selection (atomic check-and-track), not in this guard.
...
- `checkCostLimitsWithLease()` - Lease-based cost limit checking (lines 1311-1408)
...
All Lua scripts for cost tracking and session management are defined in `/Users/ding/Github/claude-code-hub/src/lib/redis/lua-scripts.ts`. The lease decrement script is defined inline in `/Users/ding/Github/claude-code-hub/src/lib/rate-limit/lease-service.ts`.
...
## 6. Cost-Based Limiting (5h/Daily/Weekly/Monthly)
...
- `RateLimitService.checkCostLimitsWithLease()` - Lease-based cost limit checking
...
*Report generated from analysis of claude-code-hub codebase - Round 2 (Verified)*

## Verification Summary

This document has been verified against the actual codebase:

| Claim | Status | Notes |
|-------|--------|-------|
| 13-step check order | **Corrected** | Actually 12 steps in guard; Provider concurrent check is done separately during provider selection |
| CHECK_AND_TRACK_SESSION Lua | Verified | Lines 26-60 in lua-scripts.ts |
| TRACK_COST_5H_ROLLING_WINDOW Lua | Verified | Lines 121-155 in lua-scripts.ts |
| TRACK_COST_DAILY_ROLLING_WINDOW Lua | Verified | Lines 208-242 in lua-scripts.ts |
| GET_COST_5H_ROLLING_WINDOW Lua | Verified | Lines 170-189 in lua-scripts.ts |
| GET_COST_DAILY_ROLLING_WINDOW Lua | Verified | Lines 257-276 in lua-scripts.ts |
| Lease decrement Lua | Verified | Lines 283-313 in lease-service.ts (inline) |
| RPM sliding window | Verified | Uses ZSET with 60s window, 2min TTL |
| Cost-based limiting | Verified | 5h rolling, daily (fixed/rolling), weekly, monthly |
| Fail-Open behavior | Verified | All checks return `{allowed: true}` on Redis failure |
