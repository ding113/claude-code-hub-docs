...
1. **性能优化**：通过缓存高频访问数据（供应商配置、系统设置、会话状态）减少数据库查询
2. **数据一致性**：使用 Redis Pub/Sub 实现跨实例缓存失效同步，确保多实例部署时数据一致性
3. **Fail-Open 策略**：Redis 不可用时自动降级，保证服务可用性
4. **灵活配置**：支持通过环境变量调整各类缓存的 TTL 时长
5. **内存安全**：设置合理的 TTL 和兜底过期时间，防止内存无限增长
...
### 3.2 Session 相关 Redis Key 及 TTL
