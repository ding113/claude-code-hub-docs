# 脚本部署 - Code Exploration Report (Round 2 - Verified)
...
**Important**: The generated docker-compose.yaml differs from the base `/Users/ding/Github/claude-code-hub/docker-compose.yaml`:
- Uses hardcoded container names with random suffix for service discovery (e.g., `claude-code-hub-db-${SUFFIX}`)
- App internal port is exposed as `${APP_PORT}:${APP_PORT}` (not `23000:3000` like base file)
- DSN uses suffixed container names: `postgresql://${DB_USER}:${DB_PASSWORD}@claude-code-hub-db-${SUFFIX}:5432/${DB_NAME}`
- Redis URL uses suffixed container names: `redis://claude-code-hub-redis-${SUFFIX}:6379`

#### 2. .env File

Location: `${DEPLOY_DIR}/.env` (permissions: 600/rw------- on Linux/macOS, ACL restricted on Windows)
...
APP_PORT=23000
APP_URL=https://<domain> (if domain specified, empty otherwise)
AUTO_MIGRATE=true
...
**Note**: The `.env` file generated by deployment scripts is a minimal subset. See `.env.example` for all available configuration options.
...
Variables generated by deployment scripts (minimal set):
...
Additional variables available in `.env.example` (must be added manually if needed):

| Variable | Default | Description |
|----------|---------|-------------|
| `DSN` | - | Full PostgreSQL connection string (overrides DB_* vars) |
| `DB_POOL_MAX` | `20` (prod) / `10` (dev) | Connection pool max size |
| `DB_POOL_IDLE_TIMEOUT` | `20` | Idle connection timeout (seconds) |
| `DB_POOL_CONNECT_TIMEOUT` | `10` | Connection timeout (seconds) |
| `MESSAGE_REQUEST_WRITE_MODE` | `async` | Request log write mode: async/sync |
| `MESSAGE_REQUEST_ASYNC_FLUSH_INTERVAL_MS` | `250` | Async flush interval |
| `MESSAGE_REQUEST_ASYNC_BATCH_SIZE` | `200` | Async batch size |
| `MESSAGE_REQUEST_ASYNC_MAX_PENDING` | `5000` | Max pending async writes |
| `API_TEST_TIMEOUT_MS` | `15000` | API test timeout (5000-120000) |
| `REDIS_URL` | `redis://localhost:6379` | Redis connection URL |
| `REDIS_TLS_REJECT_UNAUTHORIZED` | `true` | Verify Redis TLS certs |
| `FETCH_CONNECT_TIMEOUT` | `30000` | TCP connection timeout (ms) |
| `FETCH_HEADERS_TIMEOUT` | `600000` | Response headers timeout (ms) |
| `FETCH_BODY_TIMEOUT` | `600000` | Response body timeout (ms) |
| `MAX_RETRY_ATTEMPTS_DEFAULT` | `2` | Default retry attempts (1-10) |
| `ENABLE_PROVIDER_CACHE` | `true` | Enable provider process-level cache |
| `ENABLE_SMART_PROBING` | `false` | Enable smart circuit breaker probing |
| `PROBE_INTERVAL_MS` | `30000` | Probe interval (ms) |
| `PROBE_TIMEOUT_MS` | `5000` | Probe timeout (ms) |
| `ENDPOINT_PROBE_INTERVAL_MS` | `60000` | Endpoint probe base interval |
| `ENDPOINT_PROBE_TIMEOUT_MS` | `5000` | Endpoint probe timeout |
| `ENDPOINT_PROBE_CONCURRENCY` | `10` | Endpoint probe concurrency |
| `ENDPOINT_PROBE_LOG_RETENTION_DAYS` | `1` | Probe log retention |
...
**Linux**: Script automatically downloads and installs Docker via `get.docker.com`. Requires root privileges. Adds `$SUDO_USER` to `docker` group after installation.
...
- **Linux**: Uses `ip addr` or `ifconfig`, filters loopback (127.x), Docker (172.17.x), and link-local (169.254.x) addresses
...
1. **Admin Token**: 32-character cryptographically secure random string (openssl or /dev/urandom)
2. **Database Password**: 24-character cryptographically secure random string
3. **File Permissions**: `.env` file set to 600 (owner read/write only) on Unix; ACL restricted on Windows
...
The following corrections and additions were made based on code verification:

1. **Added**: Complete environment variable reference from `.env.example` (was incomplete in round1)
2. **Added**: Note about differences between generated docker-compose.yaml and base file
3. **Added**: Clarification that DSN and Redis URL in generated compose use suffixed container names
4. **Added**: Windows ACL permission details for `.env` file
5. **Added**: Docker group addition for Linux sudo users after installation
6. **Added**: More specific network address filtering details (excludes 127.x, 172.17.x, 169.254.x)
7. **Added**: Note that `.env` generated by scripts is a minimal subset of full configuration

---

*Report verified against actual source code at `/Users/ding/Github/claude-code-hub`. All paths reference the source repository.*
