# User Expiration Management - Round 2 Verified Draft
...
The 72-hour threshold for "expiring soon" is defined as a constant:

```typescript
// From: /Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/_components/user/user-key-table-row.tsx (line 84)
const EXPIRING_SOON_MS = 72 * 60 * 60 * 1000; // 72 hours in milliseconds
```
...
```typescript
// From: /Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/_components/user/forms/quick-renew-dialog.tsx (lines 85-111)
...
export const keys = pgTable('keys', {
  // ... other fields ...
  isEnabled: boolean('is_enabled').default(true),  // Note: nullable, unlike users
  expiresAt: timestamp('expires_at', { withTimezone: true }),
  // ... other fields ...
});
```

**Key differences between users and keys tables:**
- Users table: `isEnabled` is `NOT NULL DEFAULT true`
- Keys table: `isEnabled` is nullable with `DEFAULT true` only
- Only users table has the composite index on `(isEnabled, expiresAt)`
...
```typescript
// From: /Users/ding/Github/claude-code-hub/src/lib/utils/date-input.ts (lines 25-63)
export function parseDateInputAsTimezone(input: string, timezone: string): Date {
  // Date-only format (YYYY-MM-DD): interpreted as end-of-day 23:59:59 in timezone
  if (/^\d{4}-\d{2}-\d{2}$/.test(input)) {
    const localDateTime = parse(`${input} 23:59:59`, "yyyy-MM-dd HH:mm:ss", new Date());
    return fromZonedTime(localDateTime, timezone);  // Converts to UTC
  }

  // Has timezone designator (Z or offset): parse directly as absolute instant
  const hasTimezoneDesignator = /([zZ]|[+-]\d{2}:?\d{2})$/.test(input);
  if (hasTimezoneDesignator) {
    return new Date(input);  // Already UTC
  }

  // ISO datetime without timezone: treat as timezone local time, convert to UTC
  const localDate = new Date(input);
  return fromZonedTime(localDate, timezone);  // Converts to UTC
}
```
...
```typescript
// From: /Users/ding/Github/claude-code-hub/src/app/[locale]/dashboard/_components/user/user-management-table.tsx (lines 351-392)
...
This round2 draft has been verified against the actual claude-code-hub codebase. All code snippets reference absolute file paths in the source project.

### Corrections from Round 1

1. **Line numbers**: Some line numbers in round1 were approximate; round2 provides accurate line numbers
2. **Index naming**: The actual database index name is `idx_users_enabled_expires_at` (TypeScript variable is `usersEnabledExpiresAtIdx`)
3. **Keys table `isEnabled`**: Unlike users table, keys table has `isEnabled` without `.notNull()`, making it nullable
4. **Translation structure**: The actual translation keys have slightly different nesting than shown in round1
5. **Quick renew dialog**: The actual implementation has more robust error handling with `try/finally` blocks
6. **Date input parsing**: The actual `parseDateInputAsTimezone` function handles three different input formats (date-only, with timezone, without timezone)

*This draft was verified by exploring the actual claude-code-hub codebase using council agents and direct file reading.*
