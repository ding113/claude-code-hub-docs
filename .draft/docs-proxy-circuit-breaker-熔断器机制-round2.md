...
HALF-OPEN -> OPEN: When failureCount >= failureThreshold during half-open
```

**Important**: When transitioning to HALF-OPEN, the `failureCount` is reset to 0. If a failure occurs during half-open, it starts counting from 0 again. The circuit re-opens when `failureCount` reaches `failureThreshold` again (not just on a single failure).
...
**Note**: Endpoint-level circuit breaker uses hardcoded defaults and does not support per-endpoint configuration via database. All endpoints share the same `DEFAULT_ENDPOINT_CIRCUIT_BREAKER_CONFIG`.
...
- Default auto-open duration: 60 seconds (configurable via `openDurationMs` parameter)
...
```typescript
DEFAULT_ENDPOINT_CIRCUIT_BREAKER_CONFIG: EndpointCircuitBreakerConfig = {
...
**Note**: Config is cached in memory for 5 minutes (`CONFIG_CACHE_TTL`). No TTL is set on Redis config keys (they persist until explicitly cleared).
...
1. **Redis Check on Non-Closed States**: When a provider is in OPEN or HALF-OPEN state, the system always checks Redis on access to ensure external resets take effect immediately
...
When disabled:
- Provider is still added to `failedProviderIds` (to avoid re-selection in current request)
- But `recordFailure()` is not called
...
### 8. Half-Open Failure Count Reset

When transitioning from OPEN to HALF-OPEN:
- `failureCount` is reset to 0
- `halfOpenSuccessCount` is reset to 0
- `circuitOpenUntil` is cleared

This means during HALF-OPEN, failures start counting from 0. The circuit only re-opens if `failureCount` reaches `failureThreshold` again.
