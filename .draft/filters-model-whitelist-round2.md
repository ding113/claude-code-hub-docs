# Model Whitelist (模型白名单) - Round 2 Verified Draft

## Verification Status

All claims have been verified against the actual codebase at `/Users/ding/Github/claude-code-hub/`.

### Corrections from Round 1

1. **Model name pattern validation**: Round 1 claimed the pattern `/^[a-zA-Z0-9._:/-]+$/` is enforced in validation schemas. **CORRECTION**: This pattern is only enforced in the UI component (`access-restrictions-section.tsx`), not in the backend validation schema. The backend only validates max length (64 chars) and max count (50 models).

2. **Provider-level matching**: Clarified that provider-level `allowedModels` matching uses `includes()` which is **case-sensitive**, unlike user-level matching which is case-insensitive.

3. **Code snippets**: Updated `providerSupportsModel` to include the cross-type proxy support comments.
...
- Non-empty = only allow listed models (exact match, **case-sensitive**)
...
**User-Level Whitelist** (`src/drizzle/schema.ts`, lines 71-73):
...
**Provider-Level Whitelist** (`src/drizzle/schema.ts`, lines 184-188):
...
**User Interface** (`src/types/user.ts`, lines 30-31):
...
**Provider Interface** (`src/types/provider.ts`, lines 62-66):
...
**User-Level Validation** (`src/lib/validation/schemas.ts`, lines 156-161):
...
- **Note**: No pattern validation at backend level; pattern validation (`/^[a-zA-Z0-9._:/-]+$/`) is only enforced in UI

**Provider-Level Validation** (`src/lib/validation/schemas.ts`, line 384):
...
**Provider Management** (`src/actions/providers.ts`):

Providers can be configured with `allowed_models` field through `addProvider()` and `editProvider()` actions.

## Code Implementation Details

### Model Guard Implementation

**File**: `src/app/v1/_lib/proxy/model-guard.ts`
...
**File**: `src/app/v1/_lib/proxy/provider-selector.ts`, lines 115-172
...
**File**: `src/app/v1/_lib/models/available-models.ts`, lines 238-244
...
**File**: `src/app/v1/_lib/proxy/guard-pipeline.ts`, lines 65-70, 172-203
...
**File**: `src/app/[locale]/dashboard/_components/user/forms/access-restrictions-section.tsx`
...
- Model name pattern validation (UI-only): `/^[a-zA-Z0-9._:/-]+$/`
- Duplicate detection
- Maximum 50 models limit
- Suggestions support from provider configurations

### Provider Routing Section

**File**: `src/app/[locale]/settings/providers/_components/forms/provider-form/sections/routing-section.tsx`, lines 153-236

The provider form includes a Model Whitelist configuration section with:
- `ModelMultiSelect` component for selecting allowed models
- Visual display of selected models (badges)
- Count indicator for additional models ("+N more")
- Context-aware help text based on provider type
- Support for manual model name entry
...
**User-Level Matching**: **Case-insensitive**
- User whitelist: `["Claude-3-Opus"]` will match request for `"claude-3-opus"`

**Provider-Level Matching**: **Case-sensitive**
- Provider whitelist uses `includes()` which is case-sensitive
- `["claude-3-opus"]` will NOT match `"Claude-3-Opus"`

### 3. Model Name Validation

**Backend Validation** (validation schemas):
- Max length: 64 characters per model name
- Max count: 50 models per user

**Frontend Validation** (UI component only):
- Pattern: `/^[a-zA-Z0-9._:/-]+$/`
...
### 11. Cross-Type Proxy Support

Provider-level `allowedModels` supports cross-type proxy scenarios:
- A Claude-type provider can declare support for non-Claude models via `allowedModels` or `modelRedirects`
- This enables routing `gemini-*` requests through a Claude provider that has model redirects configured
- The `explicitlyDeclared` check in `providerSupportsModel` handles this case
...
1. **Model Guard**: `src/app/v1/_lib/proxy/model-guard.ts`
2. **Guard Pipeline**: `src/app/v1/_lib/proxy/guard-pipeline.ts`
3. **Provider Selector**: `src/app/v1/_lib/proxy/provider-selector.ts`
4. **Available Models**: `src/app/v1/_lib/models/available-models.ts`
5. **Database Schema**: `src/drizzle/schema.ts`
6. **User Types**: `src/types/user.ts`
7. **Provider Types**: `src/types/provider.ts`
8. **Validation Schemas**: `src/lib/validation/schemas.ts`
9. **User Repository**: `src/repository/user.ts`
10. **Provider Repository**: `src/repository/provider.ts`

### UI Components

1. **Access Restrictions Section**: `src/app/[locale]/dashboard/_components/user/forms/access-restrictions-section.tsx`
2. **Provider Routing Section**: `src/app/[locale]/settings/providers/_components/forms/provider-form/sections/routing-section.tsx`
3. **User Form**: `src/app/[locale]/dashboard/_components/user/forms/user-form.tsx`
4. **Provider Form**: `src/app/[locale]/settings/providers/_components/forms/provider-form/index.tsx`

### Actions

1. **User Actions**: `src/actions/users.ts`
2. **Provider Actions**: `src/actions/providers.ts`

---

*This document is a Round 2 verified draft for the Model Whitelist documentation. All code snippets and file paths have been verified against the actual codebase.*
