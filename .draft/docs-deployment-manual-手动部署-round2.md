...
Claude Code Hub (CCH) 是一个基于 Next.js 16 + Hono + PostgreSQL + Redis 构建的智能 AI API 代理中转服务平台。项目支持多种部署方式，其中手动部署（Manual Deployment）提供了最大的灵活性和控制权，适合有特定基础设施需求或需要自定义部署环境的用户。
...
根据 `/Users/ding/Github/claude-code-hub/package.json` 和 `/Users/ding/Github/claude-code-hub/next.config.ts`：
...
4. 构建时需要设置 `CI=true` 跳过 instrumentation 的数据库初始化
5. DSN 和 REDIS_URL 可以是占位符，不会被实际使用
...
- 生产环境（NODE_ENV=production）启用完整初始化流程
...
| 变量名 | 类型 | 必需 | 说明 |
|--------|------|------|------|
| `ADMIN_TOKEN` | string | 是 | 后台登录令牌，部署前必须修改，不能为 "change-me" |
| `DSN` | string | 是 | PostgreSQL 连接字符串，格式：`postgres://user:pass@host:5432/db` 或 `postgresql://` |
...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `AUTO_MIGRATE` | `true` | 启动时自动执行数据库迁移，设置为 `"false"` 或 `"0"` 可禁用 |
...
根据 `/Users/ding/Github/claude-code-hub/src/lib/config/env.schema.ts` 第 90 行：

```typescript
AUTO_MIGRATE: z.string().default("true").transform(booleanTransform),
```

其中 `booleanTransform` 函数定义（第 10 行）：
```typescript
...
- **默认启用**：任何非 `"false"` 且非 `"0"` 的值（包括 undefined、空字符串、`"true"`、`"1"`）都会启用自动迁移
...
重要说明：
- 构建阶段使用 Bun（快速）
- Docker 运行阶段使用 Node.js（稳定，避免内存泄漏）
- 手动部署时可以使用 `bun run start` 运行，但需监控流式响应场景的内存使用
...
1. **构建-运行分离**：使用 `bun run build` 生成 standalone 输出，然后使用 `bun run start` 启动
...
手动部署适合有一定运维经验的用户，需要对 PostgreSQL、Redis 和 Bun/Node.js 环境有基本了解。对于快速部署或测试环境，建议使用 Docker Compose 或一键部署脚本。
