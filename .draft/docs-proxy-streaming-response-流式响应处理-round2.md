...
claude-code-hub 项目实现了一套完善的流式响应处理系统，支持 Server-Sent Events (SSE) 协议，用于实时传输 AI 模型的流式输出。本文档详细介绍流式响应的技术架构、实现细节和数据流。

## 目录

1. [SSE 协议实现](#sse-协议实现)
2. [流检测与路由](#流检测与路由)
3. [块处理与缓冲区管理](#块处理与缓冲区管理)
4. [客户端-服务端流式流程](#客户端-服务端流式流程)
5. [流中的格式转换](#流中的格式转换)
6. [流式上下文中的错误处理](#流式上下文中的错误处理)
7. [空闲超时与流生命周期](#空闲超时与流生命周期)
8. [响应整流集成](#响应整流集成)
9. [Node.js 流到 Web Stream 转换](#nodejs-流到-web-stream-转换)
...
项目通过 `src/lib/utils/sse.ts` 提供健壮的 SSE 解析能力。该模块处理标准的 SSE 格式，事件通过双换行符分隔，由 `event:` 和 `data:` 字段组成。
...
```typescript
// 安全保护：如果上游长时间不输出换行，buffer 会持续增长
// 达到上限后降级为透传模式，避免 DoS 风险
...
当供应商格式与客户端期望格式不同时，系统使用 `TransformStream` 进行实时转换：
...
`flush` 方法确保流结束时缓冲区中剩余的部分数据得到正确处理。
...
export enum ErrorCategory {
  PROVIDER_ERROR,           // 供应商问题（4xx/5xx HTTP 错误）
  SYSTEM_ERROR,             // 系统/网络问题（fetch 网络异常）
  CLIENT_ABORT,             // 客户端主动中断
  NON_RETRYABLE_CLIENT_ERROR, // 客户端输入错误
  RESOURCE_NOT_FOUND,       // 上游 404 错误
}
```
...
*本文档基于 claude-code-hub 项目的实际实现编写。*
